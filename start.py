    ...
    templates=[
        {
            'name': 'Card 1',
            'qfmt': '{{Word}}',
            'afmt': '<b>{{FrontSide}}: </b> {{Answer}} <br> <b> Example use: </b> '
                    '<span id="ex">{{Examples}}</span> '
                    '<script>'
                    'var str = document.getElementById("ex").innerHTML; '
                    'arr1 = str.split(","); '
                    'var content = document.getElementById("ex").innerHTML = arr1[Math.floor(Math.random() * arr1.length)];'
                    '</script>'
        }]
)
